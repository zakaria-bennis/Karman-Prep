...
async function fixAndReuploadCsv(localPath, r2Key) {
  ...
  for (const row of data) {
    ...
    if (before !== after) { row[domainIdx] = after; fixed++; }
  }
...
async function main() {
  ...
  const targets = (jobs || []).filter((j) =>
    j.csv_storage_paths && Object.keys(j.csv_storage_paths).length > 0
    && j.imported_counts && Object.keys(j.imported_counts).length > 0
  );
  ...
  for (const job of targets) {
    ...
    for (const c of candidates) {
      ...
      if (existsSync(p)) { extractDir = p; break; }
    }
    if (!extractDir) {
    ...
    for (const [csvKey, r2Path] of Object.entries(job.csv_storage_paths)) {
      if (!r2Path) continue;
      const localFile = csvKey === "questions"
        ? "questions.csv"
      ...
      if (result) {
        console.log(`    ${csvKey}: ${result.fixed}/${result.rows} rows had domain dashes → fixed + reuploaded`);
  ...
  console.log("  curl -X POST $APP_URL/api/cron/ingest-csv-inbox -H 'authorization: Bearer $CRON_SECRET'");
