...
// v2 phase 1: --source-pdf <filename> filters --from-db reads to just
// the rows from one PDF. Equivalent to setting FILTER_SOURCE_PDF env
// but exposed as a CLI flag so the orchestrator can pass it directly.
const SOURCE_PDF_IDX = args.indexOf("--source-pdf");
const SOURCE_PDF_ARG =
  SOURCE_PDF_IDX >= 0 && args[SOURCE_PDF_IDX + 1] ? args[SOURCE_PDF_IDX + 1] : null;
const SOURCE_PDF = SOURCE_PDF_ARG ?? process.env.FILTER_SOURCE_PDF ?? null;
// csvPaths is positional args. Exclude --source-pdf's VALUE (the
// filename) so it isn't treated as a CSV path.
const csvPaths = args.filter((a, i) => {
  if (a.startsWith("--")) return false;
  if (i > 0 && args[i - 1] === "--source-pdf") return false;
  return true;
});
...
async function loadFromDb() {
  ...
  // SOURCE_PDF resolves either --source-pdf CLI arg OR
  // FILTER_SOURCE_PDF env var. The CLI version is the orchestrator's
  // single-PDF-targeting path (v2 phase 1); the env var stays for
  // backwards compatibility with the older workflow_dispatch inputs.
  if (SOURCE_PDF) {
    q = q.eq("source_pdf", SOURCE_PDF);
...
async function main() {
  ...
  if (FROM_DB) {
    // v2 phase 1: write append-only grader_runs FIRST (the new audit
    // truth) then update the latest-summary grader_votes JSONB. If
    // grader_runs writes fail mid-batch the JSONB still updates so
    // the UI badges keep working — the audit log just has gaps.
    await persistGraderRunsToDb(results);
...
// ── v2 phase 1: append-only grader_runs log ─────────────────────
// Writes one row per (question, model) for every pass that fired.
// All rows from this invocation share a single run_group_id so a
// later admin can SELECT WHERE run_group_id = X to see the full
// cascade for a sweep. Never overwrites prior runs — re-grading
// the same question produces NEW rows with a NEW group id.
async function persistGraderRunsToDb(results) {
  const { createClient } = await import("@supabase/supabase-js");
  const supa = createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL,
    process.env.SUPABASE_SERVICE_ROLE_KEY,
  ...
  // One UUID per sweep — generated by Postgres via gen_random_uuid()
  // but we want a known id to share across rows, so generate locally.
  const runGroupId =
    typeof crypto !== "undefined" && crypto.randomUUID
      ? crypto.randomUUID()
      : (await import("node:crypto")).randomUUID();
  console.log("");
  console.log(`Writing grader_runs (run_group_id=${runGroupId})…`);

  const rowsToInsert = [];
  for (const r of results) {
    if (!r.id || r.verdict === "skip_no_text") continue;
    const stored = r.stored ?? null;

    // Pass 1 — one row per voter that actually ran.
    for (const v of r.pass1?.votes ?? []) {
      if (!v.ok) continue; // skip transport errors; raw response not useful
      const role =
        v.voter === "flash"
          ? "gemini_flash_solver"
          : v.voter === "deepseek"
            ...
            : v.voter === "llama"
              ...
              : `${v.voter}_solver`;
      const provider =
        v.voter === "flash" ? "google" : v.voter === "deepseek" ? "openrouter" : "groq";
      const model =
        v.voter === "flash"
          ? "gemini-2.5-flash"
          : v.voter === "deepseek"
      ...
      rowsToInsert.push({
        ...
        provider,
        model,
        selected_answer: v.answer ?? null,
        normalized_answer: v.answer ?? null,
        answer_key_match: stored != null && v.answer != null ? v.answer === stored : null,
        raw_response_json: { voter: v.voter, answer: v.answer, ok: v.ok },
      });
    }

    // Pass 2 — Pro tiebreaker if it ran.
    if (r.pass2?.answer != null) {
      rowsToInsert.push({
        question_id: r.id,
        run_group_id: runGroupId,
        ...
        selected_answer: r.pass2.answer,
        normalized_answer: r.pass2.answer,
        answer_key_match: stored != null ? r.pass2.answer === stored : null,
        raw_response_json: r.pass2 ?? null,
      });
    }

    // Pass 3 — Opus arbiter if it ran.
    if (r.pass3?.answer != null) {
      rowsToInsert.push({
        question_id: r.id,
        run_group_id: runGroupId,
        ...
        selected_answer: r.pass3.answer,
        normalized_answer: r.pass3.answer,
        confidence: typeof r.pass3.confidence === "number" ? r.pass3.confidence : null,
        answer_key_match: stored != null ? r.pass3.answer === stored : null,
        reasoning_summary: r.pass3.reasoning ?? null,
        raw_response_json: r.pass3 ?? null,
      });
    }
  }
  ...
  // Insert in batches of 200 to stay well under Supabase's request
  // size limit on bulk inserts with raw JSONB payloads.
  const BATCH = 200;
  let inserted = 0;
  let errored = 0;
  for (let i = 0; i < rowsToInsert.length; i += BATCH) {
    const chunk = rowsToInsert.slice(i, i + BATCH);
    const { error } = await supa.from("grader_runs").insert(chunk);
    if (error) {
      errored += chunk.length;
      if (errored <= 3) console.log(`  ✗ batch ${i}: ${error.message}`);
    } else {
      inserted += chunk.length;
    }
  }
  console.log(`  → ${inserted} grader_runs inserted, ${errored} errored.`);
}
