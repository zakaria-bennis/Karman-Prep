...
export default function PredictedSATChart({ points, diagnosticsCount }: Props) {
  ...
  const lineFn = d3line<WeekPoint>().x((p) => x(p.weekIndex)).y((p) => y(p.scoreMid)).curve(curveMonotoneX);
  const areaFn = d3area<WeekPoint>().x((p) => x(p.weekIndex)).y0((p) => y(p.scoreLow)).y1((p) => y(p.scoreHigh)).curve(curveMonotoneX);
  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <Link href="/dashboard/student" className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-900 dark:hover:text-white">
        ...
        <div>
          ...
          <div className="flex items-baseline gap-4 flex-wrap">
            <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white tabular-nums">
              {latest ? `${latest.scoreLow}–${latest.scoreHigh}` : "—"}
            </h1>
            {delta !== 0 && latest && (
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-sm font-bold ${delta > 0 ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : "bg-rose-500/15 text-rose-600 dark:text-rose-400"}`}>
                <TrendingUp className="w-3.5 h-3.5" />
                {delta > 0 ? "+" : ""}{delta} since Week 1
              </span>
            )}
          </div>
        ...
        <div className="glass-card p-6">
          <div className="w-full overflow-x-auto">
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" style={{ minWidth: 600 }}>
              ...
              <g transform={`translate(${MARGIN.left}, ${MARGIN.top})`}>
                ...
                {yTicks.map((v) => (
                  <g key={v}>
                    <line x1={0} x2={innerW} y1={y(v)} y2={y(v)} stroke="currentColor" strokeOpacity={0.08} strokeDasharray="2 4" />
                    <text x={-10} y={y(v)} dy={3} textAnchor="end" fontSize="11" className="fill-slate-400 dark:fill-slate-500 tabular-nums">
                      {v}
                    </text>
                  </g>
                ))}
                ...
                {hasData && (
                  <path d={bandPath} fill="url(#predicted-band)" opacity={0.7} />
                )}

                {/* Midline */}
                {hasData && (
                  <path d={midPath} stroke="url(#predicted-line)" strokeWidth={3} fill="none" strokeLinecap="round" strokeLinejoin="round" />
                )}
                ...
                {points.map((p) => (
                  <g key={p.weekIndex}>
                    ...
                    {/* Tooltip hover-area */}
                    <title>
                      {p.weekLabel}: {p.scoreLow}–{p.scoreHigh} · {p.masteredSoFar} mastered · {p.source}
                    </title>
                  </g>
                ))}
          ...
          {/* Legend */}
          <div className="flex items-center gap-5 flex-wrap text-xs text-slate-500 dark:text-slate-400 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-4 h-2 rounded bg-gradient-to-r from-sky-400/30 to-pink-400/20" />
              Confidence range
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
          ...
          <div>
            <strong className="text-slate-700 dark:text-slate-300">How this is calculated:</strong> your Strata diagnostic sets a baseline score range.
            ...
            A refreshed diagnostic re-anchors the line. Real SAT prediction will get more precise as Strata collects more usage data.
          </div>
        </div>
      </div>
    </DashboardLayout>
