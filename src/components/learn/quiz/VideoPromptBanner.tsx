...
export function VideoPromptBanner({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="absolute inset-0 z-[70] flex flex-col items-center justify-center bg-night/95 p-6 backdrop-blur-sm"
    >
      <button
        onClick={onDismiss}
        className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full text-taupe hover:bg-surface/10 hover:text-ivory"
        aria-label="Dismiss"
      >
        <X className="h-5 w-5" />
      </button>
      <p className="mb-4 max-w-lg text-center text-lg font-semibold text-ivory">
        This concept might need another look before continuing.
      </p>
      <div className="aspect-video w-full max-w-3xl overflow-hidden rounded-xl border border-bronze bg-surface">
        {videoUrl ? (
