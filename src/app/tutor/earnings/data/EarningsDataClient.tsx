...
import {
  Wallet, Clock, DollarSign, CalendarClock, Mail, AlertCircle,
...
export default function EarningsDataClient({
  summary, sessions, payouts, weekly, range, studentSessionCounts,
}: Props) {
  ...
  const filteredPending  = sessions.filter((x) => x.payout_status === "pending" || x.payout_status === "requested").length;
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <select
            ...
            className="px-3 py-2 text-sm rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 focus:outline-none focus:border-blue-500"
          >
            {RANGE_ORDER.map((r) => (
        ...
        <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
          <PeriodCard icon={<CalendarClock className="w-4 h-4" />} label="Sessions" value={String(sessions.length)} />
          <PeriodCard icon={<Clock className="w-4 h-4" />}         label="Hours"    value={fmtHours(filteredHours)} />
          <PeriodCard icon={<DollarSign className="w-4 h-4" />}    label="Earned"   value={fmt.format(filteredAmount)} tone="emerald" />
          <PeriodCard
      ...
      <section>
        ...
        {sessions.length === 0 ? (
          <EmptyCard text={`No sessions in this range (${TIME_RANGE_LABELS[range]}). Try a wider window.`} />
        ) : (
            ...
            {payouts.map((p) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-slate-500 truncate">
                      {p.booking_count ?? 0} session{p.booking_count === 1 ? "" : "s"} · {fmtHours(p.total_hours)}
                      {p.payout_method && (
                        <>
                          <span className="mx-1.5">·</span>
                          <span className="uppercase tracking-wider text-[10px] font-semibold text-slate-600 dark:text-slate-400">
                ...
                <div className="text-xs text-slate-500 text-right shrink-0">
                  ...
                  {p.paid_at && <div className="text-emerald-500">Paid {timeSince(p.paid_at)} ago</div>}
                </div>
              </li>
            ))}
          </ul>
...
function PeriodCard({
  ...
  return (
    <div className={cn("rounded-lg px-3 py-2.5", tones[tone])}>
      ...
      <div className="mt-0.5 text-base sm:text-lg font-extrabold tabular-nums">{value}</div>
    </div>
  );
}
...
function SessionRow({
  ...
  const nameWithCount = (e: { id: string; first_name: string | null; last_name: string | null; email: string | null }) => {
  ...
  const forCol = isGroup ? (
    <div>
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-slate-900 dark:text-white font-medium truncate max-w-[18rem]">
      ...
      {enrolled.length > 0 && (
        <div className="mt-0.5 text-[11px] text-slate-500 truncate max-w-[18rem]">
    ...
    <span className="text-slate-900 dark:text-white font-medium truncate max-w-[16rem]">
  ...
  return (
    <tr className="border-t border-slate-200 dark:border-slate-800">
      <td className="px-4 py-2.5 text-slate-700 dark:text-slate-300 whitespace-nowrap align-top">
      ...
      <td className="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-400 align-top">
        {s.tutor_hours != null ? fmtHours(s.tutor_hours) : "—"}
      </td>
      <td className="px-4 py-2.5 text-right tabular-nums font-semibold text-slate-900 dark:text-white align-top">
...
function PlanTierBadge({ tier }: { tier: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    private:     { label: "Private",     cls: "bg-amber-100 dark:bg-amber-400/15 text-amber-700 dark:text-amber-300" },
    elite:       { label: "Elite",       cls: "bg-violet-100 dark:bg-violet-400/15 text-violet-700 dark:text-violet-300" },
    small_group: { label: "Small Group", cls: "bg-teal-100 dark:bg-teal-400/15 text-teal-700 dark:text-teal-300" },
    group:       { label: "Seminar",     cls: "bg-indigo-100 dark:bg-indigo-400/15 text-indigo-700 dark:text-indigo-300" },
  };
  const m = map[tier] ?? { label: tier, cls: "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300" };
  return (
    <span className={cn("inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider", m.cls)}>
      {m.label}
    </span>
  );
}
...
function RecapPill({ sent }: { sent: boolean }) {
  if (sent) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300">
        <Mail className="w-3 h-3" /> Sent
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
...
function PayoutStatusPill({ status, compact }: { status: string; compact?: boolean }) {
  const map: Record<string, { label: string; cls: string }> = {
    not_eligible:     { label: "—",                cls: "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-500" },
    pending:          { label: "Pending",          cls: "bg-amber-100 dark:bg-amber-400/15 text-amber-700 dark:text-amber-300" },
    requested:        { label: "Requested",        cls: "bg-blue-100 dark:bg-blue-400/15 text-blue-700 dark:text-blue-300" },
    pending_approval: { label: "Awaiting review",  cls: "bg-blue-100 dark:bg-blue-400/15 text-blue-700 dark:text-blue-300" },
  ...
  return (
    <span className={cn(
      "inline-block rounded text-[10px] font-bold uppercase tracking-wider whitespace-nowrap",
...
function EmptyCard({ text }: { text: string }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-800 px-6 py-8 text-center">
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3 mb-3 flex-wrap">
        <div className="text-xs uppercase tracking-wider font-bold text-slate-500">12-week total</div>
...
// Helpers
// ──────────────────────────────────────────────────────────
function studentDisplay(s: { first_name: string | null; last_name: string | null; email: string | null } | null): string {
  if (!s) return "—";
